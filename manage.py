...
def runserver(port):
    ...
    if os.name == "posix":
        ...
        cmd = f'$env:FLASK_APP = "backendmodular.app"; $env:FLASK_ENV = "development"; flask run --port={port}'
...
def plug_extension(name: str):
    ...
    with open(
        os.path.join(os.getcwd(), "backendmodular", "ext", f"{name}.py"), "w"
    ) as new_extension:
        ext_template = get_template("ext.pyt")
        new_extension.write(ext_template.render())

    settings["default"]["EXTENSIONS"].append(f"backendmodular.ext.{name}:init_app")
...
def plug_blueprint(name: str, templates: bool):
    ...
    init = open(
        os.path.join(os.getcwd(), "backendmodular", "blueprints", name, "__init__.py"),
        "w",
    )
    init.close()
    with open(
        os.path.join(os.getcwd(), "backendmodular", "blueprints", name, f"{name}.py"),
        "w",
    ) as blueprint:
