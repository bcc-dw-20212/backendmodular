...
@click.group()
def manage():
    '''This script manages the backendmodular project.'''
...
def runserver(port):
    ...
    cmd = ''
    if os.name == 'posix':
        cmd = f'export FLASK_APP=backendmodular.app; export FLASK_ENV=development; flask run --port={port}'
...
def plug_blueprint(name: str, templates: bool):
    ...
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    if f'backendmodular.blueprints.{name}.{name}:init_app' in settings['default']['EXTENSIONS']:
        click.echo('A blueprint with such a name seems to be already plugged.')
        exit(0)
    os.mkdir(os.path.join(os.getcwd(), 'backendmodular', 'blueprints', name))
    if templates:
        tf = os.path.join(
            os.getcwd(), 'backendmodular', 'blueprints', name, 'templates', name)
        os.makedirs(tf)
        click.echo(f"Placed this blueprint's templates under {tf}")

    init = open(os.path.join(
        os.getcwd(), 'backendmodular', 'blueprints', name, '__init__.py'), 'w')
    init.close()
    with open(os.path.join(os.getcwd(), 'backendmodular', 'blueprints', name, f'{name}.py'), 'w') as blueprint:
    ...
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    settings['default']['EXTENSIONS'].append(
        f'backendmodular.blueprints.{name}.{name}:init_app')
    with open(os.path.join(os.getcwd(), 'instance', 'settings.toml'), 'w') as f:
        f.write(toml.dumps(settings))
...
def plug_database():
    ...
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    if 'backendmodular.ext.database:init_app' in settings['default']['EXTENSIONS']:
    ...
    if os.name == 'posix':
        ...
        subprocess.run(['powershell', '-Command', cmd])
    # project.ext.database
    with open(os.path.join(os.getcwd(), 'backendmodular', 'ext', 'database.py'), 'w') as db_module:
        db_template = get_template('database.pyt')
        db_module.write(db_template.render(name='backendmodular'))
    # models.py (basic example)
    with open(os.path.join(os.getcwd(), 'backendmodular', 'models.py'), 'w') as models_file:
        mod_template = get_template('models.pyt')
        models_file.write(mod_template.render(project='backendmodular'))
    # settings.toml
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    settings['default']['EXTENSIONS'].append(
        'backendmodular.ext.database:init_app')
    ...
    cmd = ''
    if os.name == 'posix':
        cmd = f'export FLASK_APP=backendmodular.app; export FLASK_ENV=development; flask db init'
        subprocess.run(cmd, shell=True)
    elif os.name == 'nt':
        cmd = f'$env:FLASK_APP = "backendmodular.app"; $env:FLASK_ENV = "development"; flask db init'
        subprocess.run(['powershell', '-Command', cmd])
...
def db_migrate(message):
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    if 'backendmodular.ext.database:init_app' not in settings['default']['EXTENSIONS']:
